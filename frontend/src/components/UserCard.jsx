...
const UserCard = ({ user, onUserClick, onTodoClick }) => {
    ...
    return (
        <div className="bg-white rounded-3xl shadow-sm overflow-hidden border border-gray-100 hover:shadow-xl transition-all duration-300 flex flex-col h-full group/card">
            {/* Kart Başlığı (Header) */}
            <div 
                onClick={() => onUserClick && onUserClick(user.id)}
                className="p-5 border-b border-gray-50 relative cursor-pointer hover:bg-gray-50 transition-colors group"
            >
                <div className="absolute top-5 right-5 text-xs text-gray-400 font-medium group-hover:hidden">
                    {today}
                </div>
                <div className="absolute top-5 right-5 hidden group-hover:flex items-center gap-1 text-blue-500 text-xs font-bold animate-in fade-in">
                    <Eye size={14} /> Profili İncele
                </div>

                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                        <div className="h-12 w-12 rounded-full bg-gray-200 flex items-center justify-center text-xl font-bold text-gray-600 bg-gradient-to-br from-indigo-100 to-pink-100 border-2 border-white shadow-sm ring-1 ring-gray-100">
                             {/* Profil resmi yoksa baş harfi gösterelim */}
                             {user.username ? user.username.charAt(0).toUpperCase() : '?'}
                        </div>
                        <div>
                            <h3 className="font-bold text-lg text-gray-800 group-hover:text-blue-600 transition-colors">{user.username}</h3>
                            {/* Mood Rozeti */}
                            {user.daily_mood && (
                                <div className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full bg-purple-50 text-purple-600 text-xs font-medium border border-purple-100">
                                    <Smile size={12} />
                    ...
                    {user.todos && user.todos.length > 0 ? (
                        user.todos.map((todo) => (
                            <li 
                                ...
                                className="relative bg-white p-3 rounded-xl border border-gray-100 shadow-sm transition-all hover:shadow-md hover:border-blue-200 cursor-pointer active:scale-[0.98] group/item"
                            >
                                <div className="flex items-start gap-3">
                                    {/* Checkbox */}
                                    <div className={`mt-0.5 transition-colors ${todo.is_completed ? 'text-green-500' : 'text-gray-300 group-hover/item:text-blue-400'}`}>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start gap-2">
                                            <span className={`text-sm font-semibold transition-colors block leading-tight ${
                                                todo.is_completed ? 'line-through text-gray-400' : 'text-gray-700 group-hover/item:text-blue-700'
                                            }`}>
                                                {todo.title}
                                            </span>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 mt-2">
                                            <span className="text-[10px] font-medium px-2 py-0.5 rounded-md bg-gray-100 text-gray-600 border border-gray-200 capitalize">
                                                {todo.category}
                                            </span>
                                            {todo.priority && (
                                                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-md border ${
                                                    todo.priority === 'Yüksek' ? 'bg-red-50 text-red-600 border-red-100' :
                                                    todo.priority === 'Düşük' ? 'bg-green-50 text-green-600 border-green-100' : 
                                    ...
                                    {todo.reactions && todo.reactions.length > 0 && (
                                        <div className="flex flex-wrap gap-1 justify-end max-w-[35%] shrink-0">
                                            {groupReactions(todo.reactions).map((group, idx) => (
                                                <div 
                                                    key={idx}
                                                    className="group/emoji relative flex items-center gap-0.5 bg-gray-50 hover:bg-blue-50 border border-gray-200 hover:border-blue-200 rounded-full px-1.5 py-0.5 transition-colors cursor-help"
                                                >
